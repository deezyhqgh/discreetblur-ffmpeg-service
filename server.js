 ...
 *
 *   // for videos, EITHER of the following two shapes (not both):
 *   "frame_masks": [                   // shape A: one mask image per frame
 *     { "frame_index": 0, "mask_url": "https://..." },
 *     { "frame_index": 1, "mask_url": "https://..." },
 *     ...
 *   ],
 *   "mask_video_url": "https://...",   // shape B: fal.ai SAM2's actual video
 *                                       // output - a single video file where
 *                                       // pixel brightness encodes the mask
 *                                       // per frame. Extracted and aligned
 *                                       // to the source video's frames
 *                                       // internally (see processVideo.js).
 * }
 */
...
app.post('/apply-blur', requireServiceSecret, async (req, res) => {
  ...
  if (file_type === 'video') {
    const hasFrameMasks = Array.isArray(body.frame_masks) && body.frame_masks.length > 0;
    const hasMaskVideo = typeof body.mask_video_url === 'string' && body.mask_video_url.length > 0;
    if (!hasFrameMasks && !hasMaskVideo) {
      return res.status(400).json({
        error: 'video jobs require either frame_masks (non-empty array) or mask_video_url',
      });
    }
  }
...
async function runJob(body) {
  const { job_id, file_type, source_url, mask_url, frame_masks, mask_video_url, callback_url } = body;
  const blurOpts = {
  ...
  try {
    ...
    if (file_type === 'image') {
      ...
      // video - accept EITHER per-frame mask images (frame_masks) OR a
      // single mask video (mask_video_url, fal.ai's actual SAM2 video output
      // shape). processVideo() handles extracting the latter internally.
      localOutputPath = await processVideo({
        videoUrl: source_url,
        frameMasks: Array.isArray(frame_masks)
          ? frame_masks.map((fm) => ({ frameIndex: fm.frame_index, maskUrl: fm.mask_url }))
          : undefined,
        maskVideoUrl: mask_video_url || undefined,
        workDir,
        blurOpts,
      });
      fileName = 'output.mp4';
